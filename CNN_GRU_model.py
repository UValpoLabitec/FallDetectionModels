...
class CONV_GRU_model():
	"""
	A convolutional recurrent NN model implementation using GRU layers
	"""	

	def __init__(self, model="",  
				seq_length = 60, l2_lambda = 0.0001, 
	...
	def create_network(self):
		...
		self.network.add(Dropout(0.2))
		self.network.add(GRU(100, return_sequences=False))
...
model = CONV_GRU_model("conv_gru_model")
model.getData(X_train, Y_train, X_test, Y_test)
#model.train("conv_gru_model")
#model.save("conv_gru_model")
model.evaluate()		
